...
data merge block 475 18 497 {front_text:{messages:[[{"text":""}],[{"text":"Custom Random","bold":true}],[{"click_event":{"action":"run_command","command":"/scoreboard players set @e[name=menu,type=armor_stand] RoundType 1"},"text":"[disabled]","color":"dark_red","bold":true}],[{"text":""}]]}}
