...
clear @s[scores={TBtimer=149}] redstone

effect clear @s[scores={TBtimer=138}] minecraft:levitation

execute as @s[scores={TBtimer=149}] at @s run summon minecraft:armor_stand ~ ~ ~ {Small:1,Marker:1,Silent:1,Invulnerable:1,Tags:["tnttrack"],Invisible:1,ArmorItems:[{},{},{},{Count:1,id:tnt}]}
execute as @s[scores={TBtimer=149}] at @s run scoreboard players operation @e[distance=..0.1,limit=1,tag=tnttrack] tntID = @s tntID
execute as @s[scores={TBtimer=149}] at @s run tp @e[distance=..0.1,limit=1,tag=tnttrack] @p
...
scoreboard players operation @s[scores={TBtimer=137}] TBtimer -= @s trapper_swap_extra